...
class Solution {
    void traverse(Node* node, unordered_map<Node*,Node*>& link, Node*& cpy)
    {
        cpy = new Node(node->val);
        link[node] = cpy;
        for(auto& x : node->neighbors)
        {
            if(link.count(x))
            {
                cpy->neighbors.push_back(link[x]);
                continue;
            }
            Node* n = NULL;
            traverse(x,link,n);
            cpy->neighbors.push_back(n);
        }
    }
public:
