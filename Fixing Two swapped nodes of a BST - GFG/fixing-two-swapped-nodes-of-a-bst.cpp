...
class Solution {
    void help(Node* root, Node*& prev, vector<pair<Node*,Node*>>& v)
    {
        if(root==NULL) return;
        help(root->left,prev,v);
        if(prev!=NULL&&root->data<prev->data)
        {
            v.push_back(make_pair(prev,root));
        }
        prev = root;
        help(root->right,prev,v);
    }
  public:
    struct Node *correctBST(struct Node *root) {
        ...
        if(v.size()==1)
        {
            swap(v[0].first->data,v[0].second->data);
        }
        if(v.size()==2)
        {
            swap(v[0].first->data,v[1].second->data);
        }
        // for(auto& x : v)
        // {
